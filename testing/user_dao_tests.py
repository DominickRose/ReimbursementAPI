...
userDao: UserDao = UserDaoPostgres()
...
def test_3_1_get_single_user():
    result = userDao.get_single_user(testUser1.user_id)
    assert result.user_id == testUser1.user_id

def test_3_2_get_invalid_user():
    try:
        userDao.delete_user(0)
        assert False
...
def test_4_1_update_user():
    ...
    result = userDao.update_user(testUser1)
    assert result.first_name == 'John'

def test_4_2_update_nonexistent_user():
    invalid_user = User(0, '', '', '', '', '')
    try:
        userDao.update_user(invalid_user)
        assert False
    except ResourceNotFoundError as e:
        assert str(e) == "Resource with given ID 0 not found"

def test_5_1_delete_user():
    result1 = userDao.delete_user(testUser1.user_id)
    result2 = userDao.delete_user(testUser2.user_id)
    assert result1
    assert result2

def test_5_2_delete_invalid_user():
    try:
        userDao.delete_user(testUser1.user_id)
        assert False
    except ResourceNotFoundError as e:
